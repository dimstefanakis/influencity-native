...
import {DetachedHeader, HeaderPropsScrapper} from './DetachedHeader';
...
export function useToggleOpacity(prop) {
...
function ImageRender({width, height, source, onLoad, index}) {
...
export default function FullFeatured() {
  ...
  function onIndexChange(nextIndex) {
    setIndex(nextIndex);
  }
  ...
  const handler = useCallback(
    createAnimatedGestureHandler({
      ...
      onActive: (evt) => {
        ...
        bottomTranslateY.value = evt.translationY > 0 ? evt.translationY : 0;
      },
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <Animated.View style={[translateStyles, StyleSheet.absoluteFill]}>
        <StandaloneGallery
          ...
          renderPage={({item, ...rest}) => {
            if (item.type === 'image') {
              return <StandaloneGallery.ImageRenderer item={item} {...rest} />;
            }
          ...
          onDoubleTap={useCallback((isScaled) => {
            if (!isScaled) {
              hide();
            }
          }, [])}
      ...
      <Animated.View
        style={[s.bottomBar, opacityAnimatedStyles, translateYAnimatedStyles]}>
...
function CustomHeader({bottomTranslateY, headerShown}) {
